...
- The sink's SQL resolves stable ids from introspection alone. Metrics keyed by dataflow id join against `mz_compute_dataflow_global_ids_per_worker` to recover the global id, which is itself an introspection collection that advances on the replica clock. No catalog-derived collection sits on the emission path, so the sink's frontier stays on the replica clock.
