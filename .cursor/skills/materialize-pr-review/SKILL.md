...
2. **Run local checks** (from the repo root). Run these and report failures; suggest fixes where applicable.
   - **Format:** `cargo fmt --check` — if it fails, suggest running `cargo fmt`.
   - **Clippy:** `cargo clippy --workspace --all-targets` (or scope to affected crates if the workspace is slow).
   - **Lint:** `bin/lint` — catches general formatting nits, trailing whitespace, etc.
   - **Tests (optional):** If the diff touches code paths you want to verify, run `bin/cargo-test` or tests for the affected crate(s) (e.g. `cargo test -p mz-sql`). Report any failures.
3. **Apply the checklists below** to the diff: tests, code style, architecture, polish, release notes.
