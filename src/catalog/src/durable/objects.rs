...
pub struct BranchForkedTableEntry {
    ...
    /// table's shape as of `branch_ts`. Stored opaque here so the durable
    /// shape matches the catalog proto byte-for-byte; consumers decode to
    /// `mz_repr::RelationDesc` via prost.
    pub relation_desc: Vec<u8>,
    /// PostgreSQL-style OID for the branch's forked-table catalog entry.
    /// Allocated at branch-creation time and persisted so the entry
    /// materialized at bootstrap is stable across restarts.
    pub entry_oid: u32,
}
