...
impl MetricSink {
    ...
    pub fn new(debug_name: String, from: MetricSinkFrom) -> Self {
        Self { debug_name, from }
    }
}
...
mod tests {
    ...
    fn optimize_from(from: MetricSinkFrom) -> LirDataflowDescription {
        ...
        let global_mir_plan = optimizer
            .optimize(MetricSink::new("metric-sink-test".to_string(), from))
    ...
    fn optimizer_exports_one_metric_sink() {
        let df_desc = optimize_from(MetricSinkFrom::Id(TABLE_GID));
    ...
    fn optimizer_shapes_a_query_source() {
        ...
        let df_desc = optimize_from(MetricSinkFrom::Query {
            expr,
            desc: desc.clone(),
        });
        assert_one_shaped_metric_sink_export(&df_desc);
