...
impl Coordinator {
    ...
    async fn install_metric_sink(
        ...
        // The prefix is load-bearing: it isolates published families into the reserved lane and
        // supplies the leading character the row shaping's name validation relies on. A user sink's
        // prefix is checked at plan time; a curated one has no such gate, so enforce the same
        // contract here. The definition is ours, so a failure is a bug in a definition, not
        // something an operator can fix: give up on this one sink instead of publishing outside the
        // reserved lane.
...
mod tests {
    ...
    use crate::coord::metric_sink::{CURATED, CuratedMetricSink};
