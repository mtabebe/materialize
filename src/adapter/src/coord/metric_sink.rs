...
impl Coordinator {
    ...
    async fn install_metric_sink(
        ...
        // completes. To ensure the sink does not leak in this case, we need to already record it
        // in the registry here, rather than at the end of sequencing.
        ...
        self.sequence_metric_sink(cluster_id, replica_id, sink_id, definition)
            .await;
    }

    async fn sequence_metric_sink(
        &mut self,
        cluster_id: ClusterId,
        replica_id: ReplicaId,
        sink_id: GlobalId,
        ...
        let (expr, desc, dependencies) = match definition.plan_source(&catalog) {
            ...
            Err(err) => {
                ...
                self.metric_sinks.remove(&(replica_id, definition.name));
                return;
            }
        };
    ...
    fn metric_sink_optimize(
        ...
        // A transient id for the view the optimizer builds over the source query to shape its rows
        // (see `optimize::metric_sink::shape_metric_sink_source`); scoped to this dataflow.
    ...
    async fn metric_sink_finish(
        ...
        let still_wanted = self
            ...
            .is_some_and(|install| install.sink_id == sink_id);
        if !still_wanted {
            return Err(AdapterError::internal(
                "metric_sink",
                "metric sink has already been dropped",
            ));
        }
    ...
    pub(super) fn drop_metric_sinks(&mut self, replica_id: ReplicaId) {
        ...
        for (name, cluster_id, sink_id) in to_drop {
            ...
            // This can fail if sequencing hasn't finished yet for the sink. In that case
            // `metric_sink_finish` skips installing the compute collection in the first place.
            let _ = self
                .controller
                .compute
