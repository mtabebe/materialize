...
impl Coordinator {
    ...
    ///
    /// Reconciles the whole set rather than the delta: idempotent, and small enough that
    /// re-deriving it is free.
    ...
    async fn metric_sink_finish(
            ...
            .insert((replica_id, definition.name), install)
        {
            ...
            // colliding, so this is one definition installed twice on one replica, which
            // `reconcile_metric_sinks` reaches by starting an install an earlier in-flight one
            // then wins. Restore the first and abandon this one: shipping both would leak the
            // first's collection (unreachable to `drop_metric_sinks`) and register a second
