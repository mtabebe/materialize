...
pub fn describe(
    ...
    let desc = match stmt {
        ...
        Statement::PrepareFork(stmt) => ddl::describe_prepare_fork(&scx, stmt)?,
        Statement::DropFork(stmt) => ddl::describe_drop_fork(&scx, stmt)?,
...
pub fn plan(
    ...
    let plan = match stmt {
        ...
        Statement::PrepareFork(stmt) => ddl::plan_prepare_fork(scx, stmt),
        Statement::DropFork(stmt) => ddl::plan_drop_fork(scx, stmt),
...
impl<T: mz_sql_parser::ast::AstInfo> From<&Statement<T>> for StatementClassification {
    fn from(value: &Statement<T>) -> Self {
        ...
        match value {
            ...
            Statement::PrepareFork(_) => DDL,
            Statement::DropFork(_) => DDL,
