...
/// One forked table within a branch.
#[derive(Clone, Debug, PartialEq, Eq, PartialOrd, Ord, Serialize, Deserialize)]
#[cfg_attr(any(test, feature = "proptest"), derive(Arbitrary))]
pub struct BranchForkedTable {
    ...
    pub hold_reader_id: String,
    /// Prost-encoded `mz_repr::ProtoRelationDesc` bytes capturing the source
    /// table's shape (columns, types, metadata) as of `branch_ts`. Stored as
    /// opaque bytes so the catalog snapshot remains frozen even if
    /// `RelationDesc`'s in-memory representation evolves; consumers decode via
    /// prost into `mz_repr::RelationDesc`.
    pub relation_desc: Vec<u8>,
    /// PostgreSQL-style OID assigned to the branch's forked-table catalog
    /// entry. Allocated at branch-creation time and persisted here so the
    /// entry materialized at bootstrap is stable across restarts.
    pub entry_oid: u32,
}
...
/// Value stored per branch schema.
#[derive(Clone, Debug, PartialEq, Eq, PartialOrd, Ord, Serialize, Deserialize)]
#[cfg_attr(any(test, feature = "proptest"), derive(Arbitrary))]
pub struct BranchDescriptorValue {
    ...
    pub forked_tables: Vec<BranchForkedTable>,
}
...
pub struct BranchDescriptor {
    pub key: BranchDescriptorKey,
    pub value: BranchDescriptorValue,
}
