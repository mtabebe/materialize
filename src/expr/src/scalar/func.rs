...
macro_rules! func_name {
    ...
    (<$param:ident> $ty:ty => $name:literal, $($rest:tt)*) => {
        impl<$param> FuncName for $ty {
            const NAME: &'static str = $name;
        }
        func_name!($($rest)*);
    };
    ($ty:ty => $name:literal, $($rest:tt)*) => {
        impl FuncName for $ty {
            const NAME: &'static str = $name;
        }
        func_name!($($rest)*);
    };
...
func_name! {
    ...
    CastInt16ToNumeric => "cast_int16_to_numeric",
    ...
    CastStringToChar => "cast_string_to_char",
    CastStringToInt2Vector => "cast_string_to_int2_vector",
    <E> CastStringToList<E> => "cast_string_to_list",
    <E> CastStringToMap<E> => "cast_string_to_map",
    CastStringToNumeric => "cast_string_to_numeric",
    <E> CastStringToRange<E> => "cast_string_to_range",
    CastStringToTimestamp => "cast_string_to_timestamp",
    CastStringToTimestampTz => "cast_string_to_timestamp_tz",
    ...
    CastUint16ToNumeric => "cast_uint16_to_numeric",
    ...
    DatePartTimestamp => "date_part_timestamp",
    DatePartTimestampTz => "date_part_timestamp_tz",
    DateTruncTimestamp => "date_trunc_timestamp",
    DateTruncTimestampTz => "date_trunc_timestamp_tz",
    variadic::ErrorIfNull => "error_if_null",
    ExtractDate => "extract_date",
    ExtractInterval => "extract_interval",
    ExtractTime => "extract_time",
    ExtractTimestamp => "extract_timestamp",
    ExtractTimestampTz => "extract_timestamp_tz",
    IsLikeMatch => "is_like_match",
    ...
    RegexpSplitToArray => "regexp_split_to_array",
    TimezoneTime => "timezone_time",
    TimezoneTimestamp => "timezone_timestamp",
...
mod test {
    ...
    fn variant_names_unique() {
        // `from_variant_name` resolves the first variant with a matching
        // canonical name, so a duplicate name (from a typo in `func_name!` or
        // colliding function names) would silently shadow a variant.
        fn assert_unique(enum_name: &str, names: impl Iterator<Item = &'static str>) {
            let mut seen = std::collections::BTreeSet::new();
            for name in names {
                assert!(seen.insert(name), "duplicate {enum_name} name: {name}");
            }
        }
